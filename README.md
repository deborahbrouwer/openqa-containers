...
- [The web UI container](#The-web-UI-container)
    ...
    - [Start the container locally](#start-the-container-locally)
    - [Start the container as a service](#start-the-container-as-a-service)
    - [Login](#login)
...
Use the script `./openqa_web.sh` to start the container locally.

### Start the container as a service
```
sudo cp openqa-webserver.service /etc/systemd/system/
sudo useradd --system --user-group --home-dir /home/containers --create-home containers
sudo loginctl enable-linger $(id -u containers)
sudo -u containers mkdir /home/containers/openqa-webserver
podman save -o /tmp/openqa_webui.tar localhost/openqa_webui:latest
cd /tmp; sudo -u containers /usr/bin/podman load -i /tmp/openqa_webui.tar
rm /tmp/openqa_webui.tar

# make sure you have these directories/files especially the conf dir and init_openqa_web.sh
    /home/containers/openqa-webserver/data
    /home/containers/openqa-webserver/hdd
    /home/containers/openqa-webserver/iso
    /home/containers/openqa-webserver/conf
    sudo cp /home/fedora/openqa-fedora-containers/webUI/init_openqa_web.sh /home/containers/openqa-webserver/init_openqa_web.sh
    sudo chmod -R a+rwx /home/containers/openqa-webserver/data
    sudo chown -R containers: /home/containers

sudo systemctl daemon-reload
sudo systemctl start openqa-webserver

#check that the webserver is working
sudo journalctl -f
sudo systemctl status

#check that it is consuming scheduling messages
sudo -u containers /usr/bin/podman exec -itl /bin/bash -c 'cat /fedora_messaging_logs/"$(ls fedora_messaging_logs)"'
```
